...
/// **Continue Practice — Set N** (Stitch: *Continue Practice - Set 2*).
class TaxiSakerhetContinuePracticeScreen extends StatefulWidget {
  const TaxiSakerhetContinuePracticeScreen({super.key, required this.practiceSet});

  final int practiceSet;

  @override
  State<TaxiSakerhetContinuePracticeScreen> createState() => _TaxiSakerhetContinuePracticeScreenState();
}

class _TaxiSakerhetContinuePracticeScreenState extends State<TaxiSakerhetContinuePracticeScreen> {
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<int>(
      valueListenable: SakerhetPracticeRepository.instance.progressRevision,
      builder: (context, _, __) {
        ...
        if (total == 0) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (context.mounted) context.go('/taxi-sakerhet');
          });
          return const Scaffold(
            body: Center(child: CircularProgressIndicator()),
          );
  ...
  static String _dominantThemeLabel(List<TaxiPracticeQuestion> partition) {
    if (partition.isEmpty) return 'Övningsfrågor — taxi säkerhet';
    ...
    if (counts.isEmpty) return 'Övningsfrågor — taxi säkerhet';
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              // Answered states first so grid shows rätt/fel even when resume index matches.
